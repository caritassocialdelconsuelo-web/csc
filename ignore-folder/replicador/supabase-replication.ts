...
const DEFAULT_LAST_MODIFIED_FIELD = 'actualizadoEn';
const DEFAULT_DELETED_FIELD = 'registroBorrado';
...
> {
  ...
  private async pushHandler(
    ...
    if (row) {
      ...
      throw Error('No hay una fila en pushHandler');
    }
  }
  ...
  private async defaultUpdateHandler(
    ...
    Object.entries(row.assumedMasterState as WithDeletedAndAttachments<RxDocType>).forEach(
      ([field, value]) => {
